...
const ReqInfoForm = ({ list, UPC, prodDesc }) => {
  ...
  const handleUserQuestion = async (e) => {
    ...
    if (!canSend) return
    try {
      const response = await fetch("http://localhost:3000/api/sendQuestion", {
        method: "POST",
        headers: {
  ...
  return (
    <div>
      ...
      <form className="flex flex-col gap-[5px]" onSubmit={handleUserQuestion}>
        {list.map(
          (eachInput, index) =>
            ((eachInput.name !== "UPC" && eachInput.name !== "prodDesc") ||
              (UPC && prodDesc)) && (
              <div
